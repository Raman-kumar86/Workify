...
/**
 * Paginated hook for fetching reviews received by the current worker.
...
const useWorkerReviews = () => {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await axiosInstance.get(`/api/worker/reviews?page=1&limit=${LIMIT}`);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await axiosInstance.get(
        `/api/worker/reviews?page=${nextPage}&limit=${LIMIT}`
      );
      const { reviews: data, pagination } = res.data;
